...
public class CrackMain {
	static final Logger log = LogManager.getLogger(CrackMain.class);
	
	public static void main(String[] args) throws IOException, InterruptedException {
		System.out.println("--------------------------------------------------------");
		System.out.println("******暴力破解壓縮檔程式******");
		...
		System.out.println("例如輸入5，程式會從1位數開始測，一直測試到5位數");
		System.out.println("注意，輸入愈多破解時間愈久");
		...
		System.out.println();
		System.out.println("密碼位元輸入成功!!!");
		System.out.println();

		int passSizeInt = Integer.valueOf(passSize);

		// 執行破解service
		try {
			CrackService crackService = new CrackService(filePath, passRange, passSizeInt);
			crackService.action();
		} catch (IOException e) {
			log.error(e);
			e.printStackTrace();
